...
log_info "デプロイ開始: 環境=$ENVIRONMENT, プロジェクト=$PROJECT_ID"
...
SCHEDULER_JOB_NAME="restaurant-board-etl-daily"
...
gcloud scheduler jobs create http $SCHEDULER_JOB_NAME \
    ...
    --uri="https://restaurant-board-etl-[hash]-uc.a.run.app/" \
    --http-method=POST \
...
docker build -t gcr.io/$PROJECT_ID/restaurant-board-etl:latest .

log_info "Dockerイメージをプッシュ中..."
docker push gcr.io/$PROJECT_ID/restaurant-board-etl:latest

# Cloud Runにデプロイ
log_info "Cloud Runにデプロイ中..."
gcloud run deploy restaurant-board-etl \
    --image gcr.io/$PROJECT_ID/restaurant-board-etl:latest \
    ...
    --set-env-vars "PROJECT_ID=$PROJECT_ID,REGION=asia-northeast1,BQ_DATASET=rb,GCS_BUCKET=$BUCKET_NAME,LOG_LEVEL=info" \
...
echo "   gcloud run services update restaurant-board-etl --region=asia-northeast1 --set-env-vars STORES_SHEET_ID=your-sheet-id"
echo ""
echo "3. 手動テスト実行:"
echo "   gcloud run jobs execute restaurant-board-etl --region=asia-northeast1"
echo ""
echo "4. Connected Sheetsの設定:"
echo "   BigQueryテーブル: $PROJECT_ID.rb.vw_reservation_details"
